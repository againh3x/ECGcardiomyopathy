...
# ---------------- Config ----------------
ECG_PATH      = "data/machine_measurements.csv"
PATIENT_PATH  = "data/patients.csv.gz"
DIAG_PATH     = "data/diagnoses_icd.csv.gz"
N_KEEP        = 500

# Cardiomyopathy ICD codes to exclude (DCM + HCM)
icd_dcm = ['I420','I426','4255','4257', 'I255']
icd_hcm = ['4251','42511','I421','I422','42518']
cm_codes_norm = {c.replace('.','').upper() for c in (icd_dcm + icd_hcm)}

# Allowed & required exact phrases (case-insensitive, we normalize to lower)
PHRASE_SR  = "sinus rhythm"
PHRASE_NECG = "normal ecg"
ALLOWED = {PHRASE_SR, PHRASE_NECG}

# ---------------- Load ----------------
ecg = pd.read_csv(ECG_PATH)
patients = pd.read_csv(PATIENT_PATH, compression='gzip')
diag = pd.read_csv(DIAG_PATH, compression='gzip')

# ---------------- Exclude CM Subjects ----------------
diag['code_norm'] = (diag['icd_code'].astype(str)
                                  .str.replace('.','', regex=False)
                                  .str.upper())
cm_subjects = set(diag.loc[diag['code_norm'].isin(cm_codes_norm), 'subject_id'].unique())
ecg = ecg[~ecg['subject_id'].isin(cm_subjects)].copy()

# ---------------- Identify Report Columns ----------------
report_cols = [c for c in ecg.columns if c.startswith("report_")]
if not report_cols:
    raise ValueError("No columns starting with 'report_' found.")

# ---------------- Row Filter Function ----------------
def is_exact_two_required(row) -> bool:
    """
    Returns True iff:
      - Among report columns, NON-empty values (after strip/lower) form EXACTLY the set {'sinus rhythm','normal ecg'}
      - Each required phrase appears in AT LEAST one column
      - No other non-empty phrases present (e.g., 'abnormal ecg', 'borderline', etc.)
    """
    seen = []
    for col in report_cols:
        ...
        if isinstance(val, str):
            ...
            # Non-string value (rare). Treat as disqualifying if not empty
            return False
    if not seen:
        return False
    uniq = set(seen)
    # Must be exactly the two allowed phrases (no extras, no missing)
    if uniq != ALLOWED:
        return False
    # Both phrases present: already ensured by equality but guard if duplicates weirdly appear
    return (PHRASE_SR in uniq) and (PHRASE_NECG in uniq)

# Vectorized apply (could be optimized further if needed)
mask = ecg.apply(is_exact_two_required, axis=1)
filtered = ecg.loc[mask].copy()

filtered = filtered.sort_values(['subject_id','ecg_time'])
filtered = filtered.drop_duplicates(subset='subject_id', keep='first')
filtered_500 = filtered.head(N_KEEP).copy()

filtered_500['control'] = True

print(f"Total rows meeting EXACT two-phrase criterion: {filtered.shape[0]}")
print(f"Returning first {len(filtered_500)} rows.")

# ---------------- Merge with Patients ----------------
merged = filtered_500.merge(patients, on="subject_id", how="left")

print("Merged shape:", merged.shape)

# ---------------- Save (optional) ----------------
merged.to_csv("control_group.csv", index=False)
...
df = pd.read_csv('control_group.csv')
rec = pd.read_csv('data/record_list.csv')
SR = 500            # Hz
ECG_LEN = 10 * SR   # 5 000 samples / lead (10-s strip)
df = df.merge(rec[['study_id', 'path']], on='study_id', how='left')
...
df_combined.to_csv('control_group.csv', index=False)
...
def compute_vcg_features(X, Y, Z, q_on, q_off, t_on, t_off, p_on=None,p_off=None, fs=FS):
    ...
    # ------------------------------#
    ln_e3_T = np.log(max(St[2], 1e-12))               # (7) Ln µV
...
df.to_csv('control_group.csv', index=False)
...
df['age'] = np.nan
df['Male'] = np.nan
df['race_black'] = np.nan
df['race_white'] = np.nan
df['race_hispanic'] = np.nan


display(df.head(50))
df.to_csv('control_group1.csv', index=False)

df = pd.read_csv('full_cohort.csv')
control = pd.read_csv('control_group1.csv')
...
ecg_labels = [
    ...
    'Left Atrial Abnormality','PVC','RVR','SVR', 'IVCD'
]

for label in ecg_labels:
    control[label] = False

# Set 'Sinus Rhythm' to True for all rows
control['Sinus Rhythm'] = True

print(len(df.columns))
print(len(control.columns))
if set(df.columns) == set(control.columns):
    print("✅ The column names are identical (order ignored).")
else:
    print("❌ The column names differ.")

df = pd.concat([df, control], axis=0, ignore_index=True)
